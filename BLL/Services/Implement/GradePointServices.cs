...
namespace BLL.Services.Implement
{
    ...
    public class GradePointServices : IGradePointServices
	{
				...
				try
				{
					...
					foreach (var item in vm_gradepoint)
					{
						if (item.ExaminationPoint != null && item.ExaminationPoint.Any())
						{
							...
							float avg = ((item.Midterm_Grades * 2 + item.Final_Grades * 3 + (item.ExaminationPoint).Sum())  / (5+ (item.ExaminationPoint).Count())); 
						...
						{
                            var objForUpdate = await _appContext.GradePoints.FindAsync(item.Id);
							//Hàm tính điểm trung bình môn học khi không có điểm thành phần
							//float avg = ((item.Midterm_Grades * 2 + item.Final_Grades * 3 + (item.ExaminationPoint).Sum()) / (5 + (item.ExaminationPoint).Count()));
					...
					{
						recordsTotal = vm_gradepoint.Count(),
		...
		//Hàm import điểm bằng Excel
		public async Task<int> ImportFromExcel(Stream excelFileStream,int userId)
		{
			try
			{
		...
		public async Task<int> Modified_Item(int id, vm_update_gradepoint request)
		{
			try
			{
				...
				if (checkPoint != null)
				{
					...
					count = _appContext.Examinations.Where(c => c.GradePointId == id).Count();
				}
				...
				objForUpdate.Average = (sumPoint + objForUpdate.Midterm_Grades * 2 + objForUpdate.Final_Grades*3)/(5+count);
				// Không cần gọi Attach hoặc Update vì objForUpdate đã được
				// theo dõi trong DbContext nếu có sự thay đổi thì nó sẽ cập nhật vào DB
				await _appContext.SaveChangesAsync();
		...
		public async Task<string> GetSumPointWholeyear_Async(int id, int classid, string search = "")
		{
				...
				foreach (var subjectId in vm_gradepoints.Select(gp => gp.SubjectId).Distinct())
				{
					...
					foreach (var item in gradesForSubject)
					{
						...
						{
							k1 = (float)Math.Round(item.Average, 1);
						}
						if (item.Semester == (Semester)Enum.ToObject(typeof(Semester), 2))
						{
							k2 = (float)Math.Round(item.Average, 1); ;
						}
