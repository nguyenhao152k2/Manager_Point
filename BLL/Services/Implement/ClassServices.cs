    ...
    public class ClassServices : IClassServices
    {
        ...
        public async Task<int> Modified_Item(int id, vm_update_class request)
        {
            try
            {
                ...
                objForUpdate.Years = request.Years;
       ...
       //         }
            ...
            foreach (var userData in groupData)
            {
				...
				//userData.AcademicAbility = userData.TotalPoint >= 8 ? "Tốt" : userData.TotalPoint >= 6.5 ? "Khá" : userData.TotalPoint >= 5 ? "Đạt" : "Chưa đạt"; //: "Kém" userData.TotalPoint >= 3.5 ?
    //            userData.Conduct = userData.TotalPoint > 3.5? "Lên lớp" : "Lưu ban";
				userData.AcademicAbility = GetRankCN(userData.TotalPoint, userData.SubjectClasses);
            };
        ...
        // ki 2
        public async Task<string> GradePointByClass1(int idClass, int? semester)
        {
            var query = _appContext.GradePoints.Where(x => x.ClassId == idClass);
            ...
            foreach (var gradePoint in classGradePoints)
            {
                ...
                //if (semester == 2)
                //{
                //    var vm_gradepoint = _appContext.GradePoints.ProjectTo<vm_gradepoint>(_mapper.ConfigurationProvider).Where(c => c.UserId == user.Id && c.ClassId == idClass && c.Semester == (Semester)Enum.ToObject(typeof(Semester), semester)).ToList();
                //    foreach (var item in vm_gradepoint)
                //    {
                //        if (item.ExaminationPoint != null && item.ExaminationPoint.Any())
                //        {
                //            var objForUpdate = await _appContext.GradePoints.FindAsync(item.Id);
                //            float avg = ((item.Midterm_Grades * 2 + item.Final_Grades * 3 + (item.ExaminationPoint).Sum()) / ((item.ExaminationPoint).Count() + 5));
                //            objForUpdate.Average = avg;
                //            await _appContext.SaveChangesAsync();
                //        }
                //        else
            ...
            {
                userData.TotalPoint /= userData.SubjectClasses.Count();
                //userData.AcademicAbility = userData.TotalPoint >= 8 ? "Giỏi" : userData.TotalPoint >= 6.5 ? "Khá" : userData.TotalPoint >= 5 ? "Trung bình" : userData.TotalPoint >= 3.5 ? "Yếu" : "Kém";
                //userData.Conduct = userData.TotalPoint > 3.5 ? "Lên lớp" : "Lưu ban";
                userData.AcademicAbility = GetRankCN(userData.TotalPoint, userData.SubjectClasses);
            };
        ...
        //Hàm xếp loại thành tích học tập
        private string GetRankCN(float average, List<SubjectByClass> subjectByClasses)
        {
			int countAbove80 = subjectByClasses.Count(s => s.Avegare >= 8.0);
			int countAbove65 = subjectByClasses.Count(s => s.Avegare >= 6.5);
			int countAbove50 = subjectByClasses.Count(s => s.Avegare >= 5.0);
			int countBelow35 = subjectByClasses.Count(s => s.Avegare < 3.5);
			int countBelow50 = subjectByClasses.Count(s => s.Avegare < 5.0);

			// Mức Tốt
			if (countAbove80 >= 6 && average >= 6.5) // && countAbove65 == subjectByClasses.Count
            {
				return "Tốt";
			}

			// Mức Khá
			else if (countAbove65 >= 6  && average >= 5.0) //&& countAbove50 == subjectByClasses.Count
            {
				return "Khá";
			}

			// Mức Đạt
			else if (countBelow35 == 0  && countAbove50 >= 6 && average >= 5.0) //&& countBelow50 <= 1
            {
				return "Đạt";
			}

            // Mức Chưa đạt if(average < 5.0 && countBelow35 >= 1)
            else
            {
                return "Chưa đạt";
            }
