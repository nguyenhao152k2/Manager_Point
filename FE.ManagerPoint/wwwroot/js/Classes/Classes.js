        ...
        "columns": [
...

            {
                "data": null, "orderable": false, render: function (row) {
                    let edit = `<a class="link-warning" onclick="GetById(${row.Id})" style="cursor: pointer; margin-right: 20px; text-decoration: none"><i class="fa-solid fa-wrench mx-2"></i>Sửa</a>`;
                    let remove = `<a class="link-danger" onclick="Remove(${row.Id})" style="cursor: pointer; text-decoration: none;"><i class="fa-solid fa-trash mx-2"></i>Xóa</a>`;
    ...
    swal({
        ...
        if (willDelete) {
            $.ajax({
                ...
                method: "DELETE",
                success: function (res) {
                    toastr.success('Xóa thành công lớp học!');
                    $('#class_table').DataTable().ajax.reload();
                },
            });
        } else {
