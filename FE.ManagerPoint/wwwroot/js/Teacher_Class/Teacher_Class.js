    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response && response.length > 0) {
                ...
                $('#cardContainer').append('<p>Không có dữ liệu trong bảng</p>');
            }
        },
        error: function () {
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-9">
                                        ...
                                        <button id="editHomeRoom" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#updateModal">Đổi giáo viên chủ nhiệm</button>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response && response.length > 0) {
                ...
                $.each(response, function (index, teacher) {
                    ...
                    var cardHtml = `
                        <div class="card mt-3" id="teacherCard">
                            <img class="card-img-top" style="width:unset" src="${teacher.data.AvatarUrl}" alt="${teacher.data.Name}">
                            <div class="card-body">
                ...
                // Xử lý trường hợp không có dữ liệu trả về
                $('#cardTeacherSubject').append('<p>Không có dữ liệu trong bảng</p>');
            }
        },
        error: function () {
            // Xóa hết các card trước đó trước khi append mới
...
function Delete_Teacher_Subject(userId, subjectId, idClass) {
    swal({
        ...
        if (willDelete) {
            $.ajax({
                url: `https://localhost:44335/teacher_class/Remove_Item_By_IdUser_and_IdSubject?userId=${userId}&subjectId=${subjectId}&idClass=${idClass}`,
                method: "DELETE",
                success: function () {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res === "Ended") {
                ...
                toastr.warning('Lớp đã kết thúc, vui lòng kiểm tra lại!');
            }
            else {
